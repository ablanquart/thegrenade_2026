# Service Manual — MRF101 40m AM Linear Amplifier (USB-PD variant)
...
Two independent stories share the board: an RF amplifier that has existed across all
three variants, and a power subsystem unique to this one. The seam between them is
the VDD rail and the PD_GOOD enable signal.
...
idle warm-up — it should move only a few mA. On the THT variant the optional RD06HHF1
driver (Q5) has its own trimmer (RV2, ≈200 mA).
